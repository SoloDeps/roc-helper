...
// ── Ateliers de position ─────────────────────────────────────────────────────
// Un ID d'atelier porte la POSITION choisie par le joueur, pas le bâtiment :
// `capital_primary_workshop_construction_BA_1`. La traduction position →
// atelier réel vit dans `resolvers/workshops.ts`, seul propriétaire du sujet.
...
export async function getHydratedBuilding(
  ...
  // Une seule lecture du classement pour tout l'appel.
  const workshop = resolvePositionWorkshop(
    elementId,
    era,
    readWorkshopSelections(),
  );

  // Un atelier de position emprunte les données de l'atelier réel du joueur.
  ...
  // L'accordéon nomme la POSITION ; la fiche nomme l'atelier que le joueur y a
  // rangé, ou « Workshop {ÈRE} » tant qu'il n'a rien rangé.
  const workshopQty = workshop
  ...
  return {
    ...
    era,
    level,
    maxQty: workshopQty ?? levelData.max_qty ?? DEFAULT_MAX_QTY,
  ...
  Object.entries(checkedLevels).forEach(([levelKey, isChecked]) => {
    ...
    levelData.forEach((item) => {
      ...
      if (
        ...
        isRankGoodKey(normalizedResource)
      ) {
