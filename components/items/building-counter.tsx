...
function BuildingCounter({ value, onChange, min = 1, max = 999, disabled = false }: BuildingCounterProps) {
  return (
    <NumberField value={value} onChange={onChange} minValue={min} maxValue={max} isDisabled={disabled} aria-label="Building quantity">
      <Group className="relative inline-flex h-12 md:h-9 items-center overflow-hidden whitespace-nowrap rounded-sm border border-input text-sm shadow-sm">
        <Button
          slot="decrement"
        ...
        <div className="flex items-center justify-center size-12 md:size-9 px-2 bg-background-100 text-center font-medium tabular-nums select-none">
          {value}
        </div>
