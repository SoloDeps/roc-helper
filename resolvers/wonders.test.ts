...
import { computeSynergies, getTagContributions, computeTagCounts } from "./wonders";
...
describe("getTagContributions", () => {
  ...
  it("returns { naval: 2 } for Dragonship Ellidia (countsAs)", () => {
...
describe("computeSynergies", () => {
  it("LoA gets synergyCount 3 from CoR (naval×1) + DE (naval×2)", () => {
    ...
    // Libellé issu de loca.json (Base.Wonders.*_Name) depuis que les données
    // viennent du game design : « Ellidia », et non « Ellida ».
    expect(loa!.activatedBy).toContain("Dragonship Ellidia");
  });
